...
class BilateralFilterTool(BaseToolWindow):
  ...
  def open(self, image):
    ...
    self.widgets.colorscale = HScale(.05, .001, .2, .001, digits = 3, length = 320)
    hbox.pack_start(self.widgets.colorscale, False, False, 0)
    hbox = Gtk.HBox(spacing = 8)
    wbox.pack_start(hbox, False, False, 0)    
    hbox.pack_start(Gtk.Label(label = "\u03c3 space:"), False, False, 0)
    self.widgets.spacescale = HScale(5., .2, 32., .2, digits = 1, length = 320)
  ...
  def operation(self, params):
    ...
    sigcolor, sigspace = params
    return f"BilateralFilter(color = {sigcolor:.3f}, space = {sigspace:.1f} pixels)"
