...
class ColorBalanceTool(BaseToolWindow):
  ...
  def open(self, image):
    ...
    hbox = HBox()
    wbox.pack(hbox)
    self.widgets.redspin = SpinButton(1., 0., 2., 0.01, digits = 2)
    hbox.pack("Red:")
    hbox.pack(self.widgets.redspin)
    self.widgets.greenspin = SpinButton(1., 0., 2., 0.01, digits = 2)
    hbox.pack(8*" "+"Green:")
    hbox.pack(self.widgets.greenspin)
    self.widgets.bluespin = SpinButton(1., 0., 2., 0.01, digits = 2)
    hbox.pack(8*" "+"Blue:")
