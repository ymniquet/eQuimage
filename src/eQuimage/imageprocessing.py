...
from .imageprocessing_utils import failsafe_divide
...
imageprocessing_warned_about_frames = False

class eQuinoxImage(Image):
  """Image class for the Unistellar eQuinox 1 telescope. Handles the Unistellar frame."""

  WIDTH = 2240
  HEIGHT = 2240
  FRAMERADIUS = 997
  FRAMETHRESHOLD = 24/255

  warned = False

  def draw_frame_limit(self, ax = None, color = "yellow", linestyle = "--", linewidth = 1.):
    """Draw the Unistellar frame limit in axes 'ax' (gca() if None) with linestyle 'linestyle', linewidth 'linewidth' and color 'color'."""
    if ax == None: ax = plt.gca()
    theta = np.linspace(0., 2.*np.pi, 360)
    ax.plot(img.WIDTH/2.+img.FRAMERADIUS*np.cos(theta), img.HEIGHT/2.+img.FRAMERADIUS*np.sin(theta), color = color, linestyle = linestyle, linewidth = linewidth)

  def check_frame(self):
    """Return True is the image has an Unistellar frame."""
    global imageprocessing_warned_about_frames
    if not imageprocessing_warned_about_frames:
      print("#############################################################")
      print("# WARNING: Frame detection is presently only based on the   #")
      print("#          size of the image (wrt a framed eQuinox image).  #")
      print("#          This will (hopefully) be made more robust later. #")
      print("#############################################################")
      imageprocessing_warned_about_frames = True
    return self.image.shape == (3, self.HEIGHT, self.WIDTH)

  def get_frame(self):
    """Return the Unistellar frame as an image."""
    if not self.check_frame(): raise ValueError("The image is not a framed eQuinox image.")
    x = np.arange(0, self.WIDTH)-self.WIDTH/2.
    y = np.arange(0, self.HEIGHT)-self.HEIGHT/2.
    X, Y = np.meshgrid(x, y, sparse = True)
    outer = (X**2+Y**2 > self.FRAMERADIUS**2)
    mask = outer & (self.value() >= self.FRAMETHRESHOLD)
